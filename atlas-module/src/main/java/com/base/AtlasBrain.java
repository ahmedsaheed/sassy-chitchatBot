...
import static com.base.dateCoverter.getRealDate;
...
public class AtlasBrain {
    ...
    public static void weatherProgram() throws Exception {
        ...
        googleSpeechToText.streamingMicRecognize();
        if(googleSpeechToText.getResult().contains("1")){
            locationCount = 1;
        }else if(googleSpeechToText.getResult().contains("2")){
            locationCount = 2;
        }else if(googleSpeechToText.getResult().contains("3")){
            locationCount =3;
        }else if(googleSpeechToText.getResult().contains("4")){
            locationCount = 4;
        }else if(googleSpeechToText.getResult().contains("5")){
            locationCount = 5;
        }else{
        ...
        if(locationCount > 5){
            ...
            for (int i = 1; i <= locationCount; i++) {
                ...
                googleSpeechToText.streamingMicRecognize();
                String Stringdate = googleSpeechToText.getResult();
                dateCoverter.date(Stringdate);
        ...
        googleSpeechToText.streamingMicRecognize();
        AtlasBrain.userMessage = googleSpeechToText.getResult();

    }
    public static void doJob() throws Exception {
       // Scanner sc = new Scanner(System.in);
        googleTextToSpeech.TTS("Hi, I am Atlas, we could have a conversation casually. Feel at home and take a look around. The names on the top rightmost corner are those of my creators. If you want to view their github profile, toggle your mouse to the bottom rightmost of your screen. I could also look up the weather for you. To do so just say in either 'weather', 'forecast' or 'temperature'. If you want to end the conversation use 'q' or 'exit' ");
        ...
        while(true){
           userMessage = googleSpeechToText.getResult();
            ...
            if(userMessage.equalsIgnoreCase("q") || userMessage.equalsIgnoreCase("exit")){
                googleTextToSpeech.TTS("GoodBye");
